...
public class Utility
{
    ...
    [DllImport("pcwutl.dll", EntryPoint = "GetAppInformationFromCOS", CharSet = CharSet.Unicode)]
    [return: MarshalAs(UnmanagedType.U4)]
    public static extern uint GetAppInformationFromCOS(String PathToFile, StringBuilder URL, out uint CompatStatus, StringBuilder RecommendedLayer, out bool Apphelp);
    ...
    public static ArrayList GetAppInfoFromCOS(String PathToFile)
    {
        ...
        ReturnValue = GetAppInformationFromCOS(PathToFile, URL, out CompatStatus, RecommendedLayer, out Apphelp);
...
$Env:SQMLaunchMethod = 0
$Env:SQMCOSResponse = 0
$Env:SQMCOSLayers = ""
$Env:SQMAppliedLayers = ""
$Env:SQMUserAction = 0
$Env:SQMConsecutiveRuns = 0
$Env:SQMProblemSolved = 0

#Note launch method for SQM
if ($LaunchMethod -eq "ContextMenu")
{
    $Env:SQMLaunchMethod = 3
}
elseif ($LaunchMethod -eq "ControlPanel")
{
    $Env:SQMLaunchMethod = 2
}
elseif ($LaunchMethod -eq "CompatTab")
{
    $Env:SQMLaunchMethod = 1
}
elseif ($LaunchMethod -eq "PCA")
{
    $Env:SQMLaunchMethod = 4    
}
...
$Env:SQMCOSResponse = $AppInfo[0]
$Env:SQMCOSLayers = $AppInfo[2]
...
else
{
    ...
    $type::SendSqmForSession($Env:SQMLaunchMethod, $Env:SQMCOSResponse, $Env:SQMCOSLayers, "", 1, 0, 0)
}
